...
int main()
{
    ...
    mergeSort(ps,0,n-1);
    int proccess_cnt=0,spent_time=0;
    while (proccess_cnt<n){
        int mnid=-1;
        for (int i=n-1;i>=0;i--){
            if (vis[i]==0 && spent_time>=ps[i].arrival_time && (mnid==-1||ps[i].burst_time<=ps[mnid].burst_time)){
                mnid=i;
            }
        }
        if (mnid==-1){
            ...
            waiting_time[ps[mnid].id] = spent_time-ps[mnid].arrival_time;
            completion_time[ps[mnid].id] = spent_time + ps[mnid].burst_time;
            turn_around_time[ps[mnid].id]=waiting_time[ps[mnid].id]+ps[mnid].burst_time;
            vis[mnid]=1;
            spent_time=completion_time[ps[mnid].id];
            proccess_cnt++;
        }
    }
    for (int i=0;i<n;i++){
